...
import com.dsfr.dsfrmod.entity.EnergyArrow;
...
public class LXCrossbowSItem extends ProjectileWeaponItem implements Vanishable {
    private static final String TAG_CHARGED = "LX_Charged";
    private static final String TAG_CHARGED_PROJECTILES = "LX_ChargedProjectiles";
    ...
    public LXCrossbowSItem(Item.Properties properties) {
        super(properties);
    }
...
//        return containsChargedProjectile(p_40946_, Items.FIREWORK_ROCKET) ? 1.6F : 3.15F;
    ...
    private static boolean tryLoadProjectiles(LivingEntity p_40860_, ItemStack p_40861_) {
        int i = EnchantmentHelper.getTagEnchantmentLevel(Enchantments.MULTISHOT, p_40861_);
        int j = i == 0 ? 1 : 3;
...
//                return false;
//            }
            if (!loadProjectile(p_40860_, p_40861_, itemstack, k > 0, true)) {
                return false;
            }
        }
    ...
    public static float Mode(ItemStack itemStack){

        int mode = LXCrossbowSItem.getMode(itemStack);
        switch (mode){
            case 0:
                return 0.0F;
            case 1:
    ...
    public Component getDisplayName(){
        switch (this.mode){
            case 0:
                return Component.translatable("tooptip.dafrmod.ice_arrow");
            case 1:
                return Component.translatable("tooptip.dafrmod.fire_arrow");
            case 2:
                return Component.translatable("tooptip.dafrmod.electron_arrow");
        }
    ...
    public static int getMode(ItemStack itemStack){
        ...
        try {
            ...
            int m = compoundtag.getInt("mode");
            if(itemStack.getItem() instanceof LXCrossbowSItem){
                ((LXCrossbowSItem) itemStack.getItem()).mode = m;
            }
            return m;
        }catch (Exception e) {
            System.out.println("not mode tag");
            return 0;
        }
    }

    private static void setMode(ItemStack itemStack, int m){
        CompoundTag compoundTag = itemStack.getOrCreateTag();
        compoundTag.putInt("mode", m);
    }

    public static void setScoped(boolean b){
        scoped = b;
    }
    ...
    private static void addChargedProjectile(ItemStack p_40929_, ItemStack p_40930_) {
        ...
        ListTag listtag;
        if (compoundtag.contains("LX_ChargedProjectiles", 9)) {
            listtag = compoundtag.getList("LX_ChargedProjectiles", 10);
        } else {
            listtag = new ListTag();
        }
        ...
        compoundtag.put("LX_ChargedProjectiles", listtag);
    }

    private static List<ItemStack> getChargedProjectiles(ItemStack p_40942_) {
        List<ItemStack> list = Lists.newArrayList();
        CompoundTag compoundtag = p_40942_.getTag();
        if (compoundtag != null && compoundtag.contains("LX_ChargedProjectiles", 9)) {
            ListTag listtag = compoundtag.getList("LX_ChargedProjectiles", 10);
            if (listtag != null) {
    ...
    private static void clearChargedProjectiles(ItemStack p_40944_) {
        ...
        if (compoundtag != null) {
            ListTag listtag = compoundtag.getList("LX_ChargedProjectiles", 9);
            listtag.clear();
            compoundtag.put("LX_ChargedProjectiles", listtag);
        }
...
//                }
//            }
            Projectile projectile = getArrow(p_40895_, p_40896_, p_40898_, p_40899_);
    ...
    private static EnergyArrow getArrow(Level p_40915_, LivingEntity p_40916_, ItemStack p_40917_, ItemStack p_40918_) {
//        ArrowItem arrowitem = (ArrowItem)(p_40918_.getItem() instanceof ArrowItem ? p_40918_.getItem() : Items.ARROW);
        EnergyArrow energyArrow = switch (LXCrossbowSItem.getMode(p_40917_)) {
            case 0 -> new IceEA(p_40915_, p_40916_);
            case 1 -> new FireEA(p_40915_, p_40916_);
            case 2 -> new ElectronEA(p_40915_, p_40916_);
            default -> throw new IllegalStateException("Unexpected value: " + (LXCrossbowSItem.getMode(p_40917_)));
        };
        if (p_40916_ instanceof Player) {
            energyArrow.setCritArrow(true);
        }

        energyArrow.setSoundEvent(SoundEvents.CROSSBOW_HIT);
        energyArrow.setShotFromCrossbow(true);
        int i = EnchantmentHelper.getTagEnchantmentLevel(Enchantments.PIERCING, p_40917_);
        if (i > 0) {
            energyArrow.setPierceLevel((byte)i);
        }
...
//            ItemStack itemstack = list.get(0);
            p_40882_.add(Component.translatable("item.minecraft.crossbow.projectile").append(CommonComponents.SPACE).append(getDisplayName()));
